    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          for (int i = 0; i < sensor_fusion.size(); ++i) // for each car
          {
              ...
              if (lane == car_lane)
              {
                  ...
                  if ((check_car_s > car_s) && ((check_car_s-car_s) < min_gap))
                  {
                      ...
                      // try to change lanes
                      too_close = true;
                      speed_of_car_in_front = check_speed;
                  }
              }
          ...
          if (too_close)
          {
              if (lane != 0 && !car_to_left) {
                  lane -= 1;
              }
              else if (lane != 2 && !car_to_right) {
                  lane += 1;
              }
              else if (ref_vel > .224) {
          ...
          else
          {
              ...
              else if(ref_vel < 49.5) // increase speed
              {
                  ref_vel += .224;
              }
          }
          ...
          // Calclate how to break up spline points so that we travel at our
          ...
          for (int i = 1; i <= 50-prev_size; ++i) {
              double N = (target_dist/(.02*ref_vel/2.24)); // convert mph to mps
